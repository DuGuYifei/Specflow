...
`specflow workflow validate` 必须同时校验内置 workflow graph 和仓库中的结构化 workflow definition。校验范围至少包括 entry node、edge endpoint、session controller、managed node 和 `control_scope` edge。对仓库 definition，它还会输出当前 placeholder runtime compatibility。
...
Runtime 也负责从 workflow definition 派生 execution preview，包括节点执行模式、agent CLI、session policy、controller、control scope 摘要，以及被哪些 controller 管理的反向关系。Server 只透传 preview，UI 只消费 preview，避免可视化层复制 runtime 推导规则。
