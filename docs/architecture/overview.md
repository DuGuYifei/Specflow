...
当前 placeholder executor 仍按 Phase 1 节点 id 执行固定本地闭环。Server 创建 run 时会拒绝虽然 graph 校验通过、但缺少当前 executor 必需节点的 definition；未来真正配置驱动执行时，应把这层兼容性检查下沉到 runtime 调度能力中。
