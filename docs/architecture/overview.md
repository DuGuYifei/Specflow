...
Server 也提供 `/api/workflows`，把 `.specflow/workflows/*.workflow.json` 读取为 UI 可消费的 definition 列表，并附带 validation result 与当前 runtime compatibility result。UI 的 draft graph 应优先来自这个 API，而不是复制 runtime 的硬编码图。创建 run 的 API 可以接收 `workflowDefinitionId`，并把实际绑定的 definition reference 写入 run state。
