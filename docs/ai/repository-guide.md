...
UI draft graph 通过 server 的 `/api/workflows` 获取 workflow definition。调整 workflow definition 展示时，优先保持这个 API 边界稳定，并继续展示 definition validation、runtime compatibility 和 execution preview。

创建 run 时，UI/server 应传递并记录 `workflowDefinitionId`。修改 run state 时，不要丢失 `workflowDefinition` reference；它是复现某次 workflow 的最小来源信息。

Server 不应复制 runtime executor 的可执行性规则。如果某个 graph 是否能被当前 placeholder runtime 执行，应调用 `packages/runtime` 暴露的 compatibility helper，并把结果透传给 UI。

UI 不应自行推导节点执行模式、agent CLI、session controller 或 control scope 摘要。需要展示 definition 级执行信息时，应消费 runtime 派生的 execution preview。
