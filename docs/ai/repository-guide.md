...
UI draft graph 通过 server 的 `/api/workflows` 获取 workflow definition。调整 workflow definition 展示时，优先保持这个 API 边界稳定，并继续展示 definition validation 与 runtime compatibility 两类结果。
