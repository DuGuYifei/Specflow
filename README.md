...
Phase 0 仓库基础已经收尾，下一步进入 Phase 1：本地持续编码最小闭环。

当前代码仍只提供 local-first/package-first 工程骨架、静态 workflow graph validation、`.specflow` 读取能力和本地能力包占位。Phase 1 将开始实现真实 workflow run、artifact 记录、agent execution 和 repair loop。

## 技术选择

- CLI：TypeScript 和 `commander`
- Runtime：本地 workflow graph、状态机和执行占位
- Local API：本地 API / IPC 适配层占位
...
packages/local-api    本地 API / IPC 适配层占位
...
pnpm --filter @specflow/local-api build
```
