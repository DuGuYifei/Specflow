...
当前代码提供 local-first/package-first 工程骨架、静态 workflow graph validation、`.specflow` 读取能力，以及 Phase 1 的本地 placeholder workflow run、artifact 记录、repair loop、session module、mock Session Director、reviewer/control 节点和本地 Studio Console 面板。当前仍不调用真实 agent。

## 技术选择

- CLI：TypeScript 和 `commander`
- Runtime：本地 workflow graph、状态机和执行占位
- Server：本地 runtime API 和 UI 静态托管适配层
- UI：React 和 `@xyflow/react` 本地 workflow 面板
- 包管理：pnpm workspaces
- 任务编排：Turbo
- 本地工具链：mise
...
packages/server       本地 runtime API 和 UI 静态托管适配层
packages/ui           React 节点式 workflow 面板
...
`specflow ui` 会启动本地 server，托管 Studio Console 风格的 workflow 面板，并尝试打开浏览器。UI 通过本地 API 创建和观察 `.specflow/runs/` 中的 placeholder workflow run，并通过 `/api/workflows` 读取和选择结构化 workflow definition。

当前面板可以：

- 显示 workflow definition validation、runtime compatibility 和 execution preview。
- 显示节点角色、agent CLI、session 归属、Session Director / reviewer 的管理边和控制决策。
- 显示被管理节点的 controlled-by 关系。
- 在创建 run 时选择 mock review mode 和 repair attempt 上限。
- 在 run 列表中显示 workflow、review 次数、repair 次数和最新 review 状态。

创建 run 时会记录绑定的 definition 来源；mock run options 只用于本地 placeholder 调试，不写入 workflow definition。
